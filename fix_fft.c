 ...
 * Formatting, variable data size & ASM cleanup: Jean Zundel 
 * 04/22/2011 jzu@free.fr
 * 
 ...
 * fr[n],fi[n],fs[n],fj[n] are real,imaginary arrays, INPUT AND RESULT.
 * size of data = 2**m
 ...
 * fix_loud()
 *
 * Compute loudness of freq-spectrum components.
 ...
 * fix_dot()
 *
 * Dot product of two fixed arrays
 ***************************************************************************/
 ...
 * initSinewave()
 *
 * Pre-compute a 16-bit signed sine table
 ***************************************************************************/
...
#define PI 3.1415926
  ...
  for (i=0; i<N_WAVE; i++) {
    Sinewave [i] = (short) (sin (((double) i / N_WAVE ) * 2 * PI) * SHRT_MAX);
  }
  sine_done = 1;
/*
  for (i=0; i<N_WAVE; i++) {
    printf ("%7d,", Sinewave [i]);
    if (i % 8 == 7)
      printf ("\n");
  }
*/
...
int main (void) {
  ...
  fixed real [N_WAVE], imag [N_WAVE];
  int   i;

  for (i = 0;  i < N_WAVE;  i++) {
    real [i] = 10000 * cos (i * 2 * 3.1415926535 / (N_WAVE/15));
    imag [i] = 0;
  }

//  printf ("\nSMPLS\n");
//  for(i=0;i<N_WAVE;i++){printf("%6d, ",real[i]);if(i%10==9)printf("\n");}printf("\n};\n");
...
//  for(i=0;i<N_WAVE;i++){printf("%6d, ",real[i]);if(i%10==9)printf("\n");}printf("\n};\n");
  ...
  fix_fft (real, imag, LOG2_N_WAVE, 1);
